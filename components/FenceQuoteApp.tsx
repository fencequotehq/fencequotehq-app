...
function Metric({ icon, label, value }: any) {
  return (
    <div className="rounded-2xl bg-slate-800 p-4">
      {React.cloneElement(icon, { className: "mb-2 text-orange-300" })}
  ...
  return (
    <label className="grid gap-2 text-slate-300">
      ...
      <select value={value} onChange={(e) => setValue(e.target.value)} className="input">
        {Object.entries(options).map(([k, v]: any) => <option key={k} value={k}>{v.label}</option>)}
      </select>
    </label>
  );
}

function Check({ checked, setChecked, label }: any) {
  return (
    <label className="flex items-center gap-3 rounded-2xl border border-slate-700 bg-slate-950 p-4 text-slate-300">
      <input type="checkbox" checked={checked} onChange={(e) => setChecked(e.target.checked)} /> {label}
    </label>
  );
}

function Breakdown({ calc }: any) {
  return (
    <div className="mt-5 grid gap-3 text-sm text-slate-300">
      <div className="flex justify-between"><span>Base fence install</span><b>{currency(calc.lowBase)}–{currency(calc.highBase)}</b></div>
      <div className="flex justify-between"><span>Gate allowance</span><b>{currency(calc.gateCostLow)}–{currency(calc.gateCostHigh)}</b></div>
      <div className="flex justify-between"><span>Removal</span><b>{currency(calc.removalLow)}–{currency(calc.removalHigh)}</b></div>
      <div className="flex justify-between"><span>Stain/seal</span><b>{currency(calc.stainLow)}–{currency(calc.stainHigh)}</b></div>
    </div>
  );
}
...
function ProMode({ pro, updatePro, customer, updateCustomer, company, updateCompany, scopeNotes, setScopeNotes, takeoff, proEstimate, downloadProposalPDF }: any) {
  return (
    <section className="mx-auto max-w-7xl px-5 pb-12 md:px-10">
      <Card className="border-orange-400/30 bg-gradient-to-br from-slate-900 via-slate-900 to-slate-950 p-6">
        <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-orange-400/30 bg-orange-500/10 px-4 py-2 text-sm font-bold text-orange-200">
              <Hammer size={16} /> Pro Contractor Estimate Mode
            </div>
            <h2 className="text-3xl font-black">Bid Builder + Proposal PDF</h2>
            <p className="mt-2 max-w-3xl text-slate-300">Contractor-style takeoff, labor, overhead, profit, deposit, project duration, scope notes, and signed proposal PDF.</p>
          </div>
          <Button onClick={downloadProposalPDF} className="bg-orange-500 hover:bg-orange-600">
            <Download className="mr-2" size={18} /> Download Bid Proposal
          </Button>
        </div>
        <div className="grid gap-6 lg:grid-cols-3">
          <Card className="bg-slate-950 p-5">
            <h3 className="mb-4 text-xl font-black">Customer + Company</h3>
            {["name","address","city","phone","email"].map(k => (
              <input key={k} value={customer[k]} onChange={(e) => updateCustomer(k, e.target.value)} placeholder={`Customer ${k}`} className="input mb-3" />
            ))}
            <input value={company.name} onChange={(e) => updateCompany("name", e.target.value)} placeholder="Company name" className="input" />
          </Card>
          <Card className="bg-slate-950 p-5">
            <h3 className="mb-4 text-xl font-black">Pricing Controls</h3>
            {[["hourlyRate","Hourly Rate"],["crewSize","Crew Size"],["overheadPercent","Overhead %"],["profitPercent","Profit %"],["depositPercent","Deposit %"]].map(([k,l]) => (
              <label key={k} className="mb-3 grid gap-1 text-sm text-slate-300">
                {l}
                <input type="number" value={pro[k]} onChange={(e) => updatePro(k, Number(e.target.value))} className="input" />
              </label>
            ))}
          </Card>
          <Card className="border-orange-400/30 bg-orange-500/10 p-5">
            <h3 className="mb-4 text-xl font-black">Bid Summary</h3>
            {[["Material",proEstimate.materialCost],["Labor",proEstimate.laborCost],["Overhead",proEstimate.overhead],["Contingency",proEstimate.contingency],["Profit",proEstimate.profit]].map(([k,v]: any) => (
              <div key={k} className="flex justify-between text-sm text-slate-300"><span>{k}</span><b>{currency(v)}</b></div>
            ))}
            <div className="mt-3 rounded-2xl bg-slate-950 p-4">
              <p className="text-slate-400">Proposed Bid</p>
              <p className="text-3xl font-black">{currency(proEstimate.bidPrice)}</p>
            </div>
            <p className="mt-3 text-slate-300">Deposit: <b>{currency(proEstimate.deposit)}</b></p>
            <p className="text-slate-300">Duration: <b>{proEstimate.estimatedDays} day(s)</b></p>
          </Card>
        </div>
        <div className="mt-6 grid gap-6 lg:grid-cols-2">
          <Card className="bg-slate-950 p-5">
            <h3 className="mb-4 text-xl font-black">Material Takeoff Pro</h3>
            <div className="grid grid-cols-2 gap-3 text-sm md:grid-cols-4">
              {Object.entries(takeoff).map(([k,v]) => (
                <div key={k} className="rounded-2xl border border-slate-700 bg-slate-900 p-4">
          ...
          <Card className="bg-slate-950 p-5">
            <h3 className="mb-4 text-xl font-black">Scope + Terms</h3>
            <textarea value={scopeNotes} onChange={(e) => setScopeNotes(e.target.value)} className="input min-h-28" />
            <input value={pro.warranty} onChange={(e) => updatePro("warranty", e.target.value)} placeholder="Warranty" className="input mt-3" />
            <input value={pro.paymentTerms} onChange={(e) => updatePro("paymentTerms", e.target.value)} placeholder="Payment terms" className="input mt-3" />
          </Card>
        </div>
      </Card>
    </section>
...
export default function FenceQuoteApp() {
  ...
  const calc = useMemo(() => {
    ...
    const zipMultiplier =
      zip.startsWith("750") ? 1.15 :
      zip.startsWith("751") ? 1.12 :
      zip.startsWith("752") ? 1.18 :
      zip.startsWith("760") ? 1.05 :
      zip.startsWith("761") ? 1.08 :
      zip.startsWith("770") ? 1.14 :
      zip.startsWith("782") ? 1.10 :
      zip.startsWith("787") ? 1.20 :
      zip.startsWith("799") ? 0.92 : 1;
  ...
  const estimateData = {
    ...
    corners, stain: stain ? "Yes" : "No", low: currency(calc.low), high: currency(calc.high),
    confidence: `${calc.confidence}%`, proBid: currency(proEstimate.bidPrice), estimatedDays: proEstimate.estimatedDays,
  };
  ...
  function validateLead() {
    ...
    if (!lead.email.trim() || !lead.email.includes("@") || !lead.email.includes(".")) return "Please enter a valid email address.";
    return "";
  }
  ...
  function downloadEstimatePDF() {
    ...
    doc.setFont("helvetica", "bold"); doc.text("Material Takeoff", left, y); y += 7;
  ...
  function downloadProposalPDF() {
    ...
    doc.setFont("helvetica", "normal"); doc.setFontSize(10);
    line("Customer", customer.name || lead.name); line("Address", customer.address);
    line("City/ZIP", `${customer.city} ${zip}`); line("Fence", `${length} ft ${MATERIALS[material].label}`);
    y += 4; doc.setFont("helvetica", "bold"); doc.text("Proposal Summary", left, y); y += 7;
    line("Material", currency(proEstimate.materialCost)); line("Labor", currency(proEstimate.laborCost));
    line("Overhead", currency(proEstimate.overhead)); line("Profit", currency(proEstimate.profit));
    y += 2; doc.setFontSize(16); doc.setFont("helvetica", "bold");
    doc.text(`Total Bid: ${currency(proEstimate.bidPrice)}`, left, y); y += 8;
    doc.setFontSize(10); line("Deposit", currency(proEstimate.deposit));
    line("Duration", `${proEstimate.estimatedDays} working day(s)`); line("Warranty", pro.warranty);
    ...
    <main className="min-h-screen overflow-hidden bg-slate-950 text-white">
      <section className="relative px-5 py-8 md:px-10 md:py-14">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-8 md:grid-cols-2 md:items-center">
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}>
            <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-orange-400/30 bg-orange-500/10 px-4 py-2 text-sm text-orange-200">
              <Sparkles size={16} /> Fence planning software for Texas homeowners
            </div>
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              <Button onClick={() => document.getElementById("calculator")?.scrollIntoView()} className="bg-orange-500 hover:bg-orange-600">
                Start Estimate <ArrowRight className="ml-2" size={18} />
              </Button>
              <Button
                onClick={() => {
                  setMode("pro");
                  setTimeout(() => { document.getElementById("pro-mode")?.scrollIntoView({ behavior: "smooth" }); }, 200);
                }}
          ...
          <Card className="relative bg-slate-900/85 p-6">
            <div className="mb-5 flex items-start justify-between">
              <div>
                <p className="text-sm text-slate-400">Live installed estimate</p>
                <h2 className="text-3xl font-black md:text-4xl">{currency(calc.low)} – {currency(calc.high)}</h2>
              </div>
            ...
            <div className="mt-4 rounded-2xl bg-slate-800 p-4">
              ...
              <div className="flex flex-wrap gap-1 rounded-xl bg-slate-950 p-3">
                {Array.from({ length: Math.min(30, Math.ceil(length / 10)) }).map((_, i) => (
                  <div key={i} className="w-3 rounded-sm bg-gradient-to-b from-amber-500 to-amber-800 shadow" style={{ height: height === 4 ? 34 : height === 6 ? 44 : 56 }} />
                ))}
                {Array.from({ length: gates }).map((_, i) => (
                  <div key={i} className="h-12 w-7 rounded-md border-2 border-orange-400 bg-slate-700" />
                ))}
              </div>
            </div>
          </Card>
      ...
      <section id="calculator" className="mx-auto grid max-w-7xl gap-6 px-5 py-8 md:grid-cols-[1.05fr_.95fr] md:px-10">
        <Card className="p-6">
          ...
          <div className="grid gap-5">
            ...
            <label className="grid gap-2 text-slate-300">
              Fence Length: <span className="font-bold text-orange-300">{length} ft</span>
              <input type="range" min="50" max="500" step="10" value={length} onChange={(e) => setLength(Number(e.target.value))} />
            </label>
            <div className="grid grid-cols-3 gap-3">
              {[4, 6, 8].map((h) => (
                <Button key={h} onClick={() => setHeight(h)} className={height === h ? "bg-orange-500" : "bg-slate-800 hover:bg-slate-700"}>{h} ft</Button>
              ))}
            </div>
            <div className="grid gap-3 md:grid-cols-2">
              {Object.entries(MATERIALS).map(([key, item]) => (
                <button key={key} onClick={() => setMaterial(key as MaterialKey)} className={`rounded-2xl border p-4 text-left transition hover:scale-[1.01] ${material === key ? "border-orange-400 bg-orange-500/10" : "border-slate-700 bg-slate-950 hover:bg-slate-800"}`}>
                  <b>{item.label}</b>
                  <p className="text-sm text-slate-400">{currency(item.low)}–{currency(item.high)} / ft</p>
                </button>
              ))}
            </div>
            <div className="grid gap-4 md:grid-cols-2">
              <Input label="Gates" value={gates} setValue={setGates} />
              <Input label="Corners / Turns" value={corners} setValue={setCorners} />
            </div>
            <div className="grid gap-4 md:grid-cols-3">
              <Select label="Terrain" value={terrain} setValue={setTerrain} options={TERRAIN_MULTIPLIER} />
              <Select label="Install Level" value={labor} setValue={setLabor} options={LABOR_MULTIPLIER} />
              <Select label="Post System" value={postType} setValue={setPostType} options={POST_OPTIONS} />
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              <Check checked={removal} setChecked={setRemoval} label="Remove old fence" />
              <Check checked={stain} setChecked={setStain} label="Add stain/seal estimate" />
            </div>
          </div>
        </Card>
        <div className="grid gap-6">
          <Card className="border-orange-400/30 bg-gradient-to-br from-slate-900 via-slate-900 to-orange-950/40 p-6">
            <p className="text-sm text-orange-300">Estimated installed cost</p>
            <h2 className="mt-2 text-4xl font-black">{currency(calc.low)} – {currency(calc.high)}</h2>
            <Breakdown calc={calc} />
            <Button onClick={downloadEstimatePDF} className="mt-5 w-full bg-orange-500 hover:bg-orange-600">
              <Download className="mr-2" size={18} /> Download Estimate PDF
            </Button>
          </Card>
          <Card className="p-6">
      ...
      <section className="mx-auto grid max-w-7xl gap-6 px-5 pb-8 md:grid-cols-2 md:px-10">
        <Card className="p-6">
          ...
          <select value={layout} onChange={(e) => setLayout(e.target.value as any)} className="input mt-4">
            <option value="backyard">Backyard only</option>
            <option value="full">Full perimeter</option>
            <option value="pool">Pool enclosure</option>
          </select>
          <div className="mt-4 rounded-2xl bg-slate-950 p-4">
            <p className="text-slate-400">Estimated fence footage</p>
            <p className="text-3xl font-black">{lotEstimate} ft</p>
            <Button onClick={() => setLength(lotEstimate)} className="mt-3 bg-orange-500">Use This Length</Button>
          </div>
        </Card>
        <Card className="p-6">
          <div className="mb-4 flex items-center gap-2"><Mail className="text-orange-300" /><h3 className="text-xl font-black">Get Estimate + Contractor Options</h3></div>
          <input value={lead.name} onChange={(e) => updateLead("name", e.target.value)} placeholder="Full name" className="input mb-3" />
          <input value={lead.email} onChange={(e) => updateLead("email", e.target.value)} placeholder="Email" className="input mb-3" />
          <input value={lead.phone} onChange={(e) => updateLead("phone", e.target.value)} placeholder="Phone optional" className="input mb-3" />
          <input value={webhookUrl} onChange={(e) => setWebhookUrl(e.target.value)} placeholder="Optional Zapier webhook URL" className="input mb-3" />
          {leadError && <p className="mb-2 text-red-300">{leadError}</p>}
          {leadStatus && <p className="mb-2 text-emerald-300">{leadStatus}</p>}
          <Button onClick={sendLead} disabled={isSending} className="w-full bg-orange-500 hover:bg-orange-600">
            <Send className="mr-2" size={18} /> {isSending ? "Sending..." : "Send Lead"}
          </Button>
        </Card>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-5 pb-8 md:px-10">
        <Card className="p-4 md:flex md:items-center md:justify-between">
          <div>
            <p className="text-sm font-bold uppercase tracking-[0.2em] text-orange-300">Estimator Mode</p>
            <h2 className="text-2xl font-black">Homeowner calculator + contractor proposal software</h2>
          </div>
          <div className="mt-4 grid grid-cols-2 gap-2 rounded-2xl bg-slate-950 p-2 md:mt-0">
            <Button onClick={() => setMode("homeowner")} className={mode === "homeowner" ? "bg-orange-500" : "bg-slate-800"}>Homeowner</Button>
            <Button onClick={() => setMode("pro")} className={mode === "pro" ? "bg-orange-500" : "bg-slate-800"}>Pro Contractor</Button>
          </div>
        </Card>
      </section>
      ...
      <section className="mx-auto grid max-w-7xl gap-6 px-5 pb-12 md:grid-cols-3 md:px-10">
        {[
          { icon: Layers, title: "Accuracy Logic", text: "Uses length, height, material, gates, post system, removal, terrain, corners, stain, and local market multipliers." },
          { icon: ClipboardCheck, title: "Contractor-Ready", text: "Captures project details in a format a fence contractor can quickly review and bid." },
          { icon: Star, title: "Sticky Experience", text: "Visual preview, quote score, lot estimator, takeoff, and proposal PDFs keep users engaged." }
        ].map((item) => (
          <Card key={item.title} className="p-6">
            <item.icon className="mb-4 text-orange-300" />
